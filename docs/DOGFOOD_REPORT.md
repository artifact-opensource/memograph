...
- **Safe-mode fallback**: Native agent profile context preserved (dormant). Never overwritten.
...
| `/home/adam/.hermes` | 5.4 GB (47 items) | Agent gateway config |
...
- **`.hermes-tmp` permission denied**: Host-level temp-file layer shows silent drop on some writes (`.hermes-tmp.1359699`). Impact: file mutations through temp layer are unreliable. Workaround: direct file writes (like `write_file` tool) succeed. Audit trail uses direct writes, not temp layer.
